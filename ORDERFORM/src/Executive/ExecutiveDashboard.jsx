...
import { APPOINTMENTS, EXECUTIVES, ORDERS } from "../utils/endpoints";
...
const ExecutiveDashboard = () => {
    ...
    const fetchAppointmentCount = async () => {
      try {
        ...
        const assigned = res.data.filter(
          (appt) =>
            appt.status === "assigned" && appt.executive === selectedExecutive
        );

        const newCount = assigned.length;
        const storedCount =
          parseInt(localStorage.getItem("lastSeenAppointmentCount")) || 0;
  ...
  const fetchExecutiveData = async (executiveName) => {
    try {
      const ordersRes = await axios.get(`${EXECUTIVES}/${executiveName}`);
        ...
        if (Array.isArray(order.rows)) {
          order.rows.forEach((row) => {
            const deliveryDate = row.deliveryDate
              ? parseISO(row.deliveryDate)
              : null;
            const isExpired =
              deliveryDate && isBefore(deliveryDate, new Date());
  ...
  useEffect(() => {
    const fetchPendingPayments = async () => {
      try {
        const res = await axios.get(`${ORDERS}/pending-payments`);
            ...
            <PieChart>
              ...
              <Tooltip
                formatter={(value, name) => [
                  `₹${value}`,
                  name === "Extra" ? "Extra Achieved" : "Target Portion",
                ]}
              />
              <Legend />
            </PieChart>
              ...
              <PieChart>
                <Pie
                  data={[
                    ...
                    {
                      name: "Completed",
                      value: serviceData[0]?.completed || 0,
                    },
                  ]}
                  cx="50%"
                  cy="50%"
