...
function OrderForm() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const reqRes = await axios.get(REQUIREMENTS);
  ...
  const checkIfExistingClient = async (number) => {
    try {
      const res = await axios.get(`${CHECK_CLIENT}?phone=${number}`);
  ...
  const handlePaymentMethodChange = async (method) => {
    ...
    if (method === "UPI") {
      try {
        const res = await axios.get(UPI_NUMBERS);
  ...
  const handleSubmit = async () => {
    const orderData = {
      ...
      paymentDate,
      paymentMethod:
        paymentMethod === "UPI" ? `UPI - ${selectedUpi}` : paymentMethod,
      advance,
      balance,
      total,
      chequeNumber,
    ...
    try {
      await axios.post(SUBMIT, orderData);
        ...
        <div className="sidebar-content">
          <div className="nav-menu">
            ...
            <div className="nav-item" onClick={() => setActiveTab("order")}>
              <span className="nav-icon">📝</span>
              <span className="nav-text">Create Order ➕</span>
            </div>
            <div
              className="nav-item"
