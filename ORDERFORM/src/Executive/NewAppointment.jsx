...
const NewAppointment = () => {
  ...
  useEffect(() => {
    const fetchAssignedAppointments = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:5000/api/appointments/assigned/${executiveName}`);
  ...
  const handleSendStatus = (id) => {
    ...
    axios.put(`http://localhost:5000/api/appointments/${id}/status`, { status: selectedStatus })
      .then(response => {
          ...
          {appointments.map(appt => (
            ...
            >
              ...
              <div style={{ marginTop: '10px' }}>
                <div style={{ fontWeight: '600', color: '#33691e', marginBottom: '5px' }}>Status:</div>
                <select
