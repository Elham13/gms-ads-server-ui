...
function ViewOrders() {
  ...
  const handleMarkPaid = async (id) => {
    try {
      await axios.put(`${ORDERS}/${id}/mark-paid`);
  ...
  return (
    ...
    >
      <h2
        style={{ textAlign: "center", marginBottom: "20px", color: "#1a5d1a" }}
      >
        View Orders
      </h2>
            ...
            {orders.map((order, orderIndex) =>
              order.rows.filter(filterOrders(order)).map((row, rowIndex) => (
                <tr
                  ...
                  style={{
                    backgroundColor:
                      (orderIndex + rowIndex) % 2 === 0 ? "#fdfdfd" : "#f1f8f9",
                  }}
                >
                  <td>{orderIndex + 1}</td>
